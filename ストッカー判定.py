...
class CloudStorageImageFolder(Dataset):
    def __init__(self, bucket_name, prefix, transform=None):
        ...
        prefix: データセットのルートパス (例: "photos/ストッカー判定/train")
        transform: 画像前処理
        """
...
train_prefix = "photos/ストッカー判定/train"
val_prefix   = "photos/ストッカー判定/val"
test_prefix  = "photos/ストッカー判定/test"
...
# -----------------------------------------
# まずローカルに一時保存
local_model_path = "stocker.pth"
torch.save(model.state_dict(), local_model_path)

# Cloud Storageにアップロード（バケット名: trash_check_ai、Blob名: model/stocker.pth）
client = storage.Client()
bucket = client.bucket(bucket_name)
blob = bucket.blob("model/stocker.pth")
blob.upload_from_filename(local_model_path)

print("Model saved to Cloud Storage at: trash_check_ai/model/stocker.pth")
